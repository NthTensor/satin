...
fn tree_prefix_collect(b: &mut ::test::Bencher) {
    ...
    b.iter(|| {
        vec = Some(
            satin::iter::walk_tree_prefix(0u64..SIZE, |r| {
                // root is smallest
...
fn tree_postfix_collect(b: &mut ::test::Bencher) {
    ...
    b.iter(|| {
        vec = Some(
            satin::iter::walk_tree_postfix(0u64..SIZE, |r| {
                // root is largest
...
fn tree_prefix_sum(b: &mut ::test::Bencher) {
    b.iter(|| {
        let s = satin::iter::walk_tree_prefix(0u64..SIZE, |r| {
            // root is smallest
            let mid = (r.start + 1 + r.end) / 2;
            // small indices to the left, large to the right
            std::iter::once((r.start + 1)..mid)
...
fn tree_postfix_sum(b: &mut ::test::Bencher) {
    b.iter(|| {
        let s = satin::iter::walk_tree_postfix(0u64..SIZE, |r| {
            // root is smallest
            let mid = (r.start + 1 + r.end) / 2;
            // small indices to the left, large to the right
            std::iter::once((r.start + 1)..mid)
