...
//!
//! It's potentially interesting for satin::join, because the splits are
...
#[bench]
/// Compute the Fibonacci number recursively, using satin::join.
...
fn fibonacci_join_1_2(b: &mut test::Bencher) {
    fn fib(n: u32) -> u32 {
        ...
        let (a, b) = forte::join(|_| fib(n - 1), |_| fib(n - 2));
        a + b
    }
...
fn fibonacci_join_2_1(b: &mut test::Bencher) {
    fn fib(n: u32) -> u32 {
        ...
        let (a, b) = forte::join(|_| fib(n - 2), |_| fib(n - 1));
        a + b
    }
...
#[bench]
/// Compute the Fibonacci number recursively, using satin::iter::split to parallelize.
fn fibonacci_split_recursive(b: &mut test::Bencher) {
    fn fib(n: u32) -> u32 {
        use satin::iter::ParallelIterator;

        satin::iter::split(n, |n| {
            if n < 2 {
                (n, None)
            } else {
                (n - 2, Some(n - 1))
...
#[bench]
/// Compute the Fibonacci number iteratively, using satin::iter::split to parallelize.
