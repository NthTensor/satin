...
where
    ...
    R4: Send,
{
    let ((r1, r2), (r3, r4)) = forte::join(|_| forte::join(|_| f1(), |_| f2()), |_| forte::join(|_| f3(), |_| f4()));
    (r1, r2, r3, r4)
}
...
where
    ...
    R8: Send,
{
    let (((r1, r2), (r3, r4)), ((r5, r6), (r7, r8))) = forte::join(
        |_| forte::join(|_| forte::join(|_| f1(), |_| f2()), |_| forte::join(|_| f3(), |_| f4())),
        |_| forte::join(|_| forte::join(|_| f5(), |_| f6()), |_| forte::join(|_| f7(), |_| f8())),
    );
    (r1, r2, r3, r4, r5, r6, r7, r8)
}
...
fn strassen_add2_mul(a1: &[f32], a2: &[f32], b1: &[f32], b2: &[f32]) -> Vec<f32> {
    ...
    let (a, b) = forte::join(|_| rtmp_sum(a1, a2), |_| rtmp_sum(b1, b2));
...
fn strassen_sub_add_mul(a1: &[f32], a2: &[f32], b1: &[f32], b2: &[f32]) -> Vec<f32> {
    let mut dest = raw_buffer(a1.len());
    let (a, b) = forte::join(|_| rtmp_sub(a1, a2), |_| rtmp_sum(b1, b2));
    matmul_strassen(&a[..], &b[..], &mut dest[..]);
    dest
}
...
fn rcopy(src: &[f32], dest: &mut [f32]) {
    ...
    forte::join(|_| rcopy(s1, d1), |_| rcopy(s2, d2));
}
