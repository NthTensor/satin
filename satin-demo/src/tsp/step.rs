...
pub fn step<'scope, 'env: 'scope>(
    scope: &'scope Scope<'scope, 'env>,
    solver: &'env SolverCx<'env>,
) {
...
fn split_tour<'scope, 'env: 'scope>(
    scope: &'scope Scope<'scope, 'env>,
    solver: &'env SolverCx<'env>,
    element: Arc<TourPrefix>,
) {
    ...
    for next_edge in graph.edges(last_node) {
        ...
        if !element.visited(next_node) {
            ...
            if (element.prefix_weight + weight) < solver.min_tour_weight() {
                ...
                // forte's scope.spawn passes &Worker (not &Scope); capture scope directly
                scope.spawn(move |_: &forte::Worker| step(scope, solver));
            }
        }
    }
}
